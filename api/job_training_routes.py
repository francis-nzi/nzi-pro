...
def _ensure_tables(con) -> None:
    ...
    con.execute(
        ...
        CREATE TABLE IF NOT EXISTS training_documents (
          training_document_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
def _doc_payload(row) -> dict[str, Any]:
    return {
        "training_document_id": int(row[0]),
        "org_id": row[1],
        "target_type": row[2],
        "target_id": int(row[3]),
        "document_type": row[4],
...
def list_training_documents(
    ...
    with get_conn() as con:
        _ensure_tables(con)
        rows = con.execute(
            _DOC_SELECT + " WHERE org_id = ?::text AND target_type = ? AND target_id = ? ORDER BY document_type, document_name",
            [org_id, target_type, int(target_id)],
        ).fetchall()
        return {"documents": [_doc_payload(r) for r in rows]}


@router.post("/training-documents")
def create_training_document(body: dict = Body(...), _user: dict[str, str] = Depends(_current_user)):
    assert_permission(_user, "jobs.edit")
    org_id = require_org(_user)
    actor = _actor(_user)
    with get_conn() as con:
        ...
        return {"document": _doc_payload(row)}


@router.put("/training-documents/{doc_id}")
def update_training_document(doc_id: int, body: dict = Body(...), _user: dict[str, str] = Depends(_current_user)):
    assert_permission(_user, "jobs.edit")
    org_id = require_org(_user)
    actor = _actor(_user)
    with get_conn() as con:
...
@router.delete("/training-documents/{doc_id}")
def delete_training_document(doc_id: int, _user: dict[str, str] = Depends(_current_user)):
    assert_permission(_user, "jobs.edit")
    org_id = require_org(_user)
    with get_conn() as con:
        _ensure_tables(con)
        exists = con.execute(
            "SELECT 1 FROM training_documents WHERE training_document_id = ? AND org_id = ?::text",
            [int(doc_id), org_id],
        ).fetchone()
        if not exists:
            raise HTTPException(status_code=404, detail="Document not found")
        con.execute(
            "DELETE FROM training_documents WHERE training_document_id = ? AND org_id = ?::text",
            [int(doc_id), org_id],
        )
        return {"ok": True}
