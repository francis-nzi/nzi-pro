...
def _org_or_default(user: dict[str, str]) -> str:
    return require_org(user)
...
def _ensure_tables(con) -> None:
    ...
    con.execute("ALTER TABLE bd_scan_batches ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    con.execute("ALTER TABLE bd_market_companies ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    con.execute("ALTER TABLE bd_company_verifications ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    con.execute("ALTER TABLE bd_company_contacts ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    con.execute("ALTER TABLE bd_market_scores ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    con.execute("ALTER TABLE bd_leads ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    con.execute("ALTER TABLE bd_opportunities ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    con.execute("ALTER TABLE bd_ai_generated_leads ADD COLUMN IF NOT EXISTS org_id VARCHAR")
    ...
    *,
    ...
    row = con.execute(
        ...
        INSERT INTO bd_scan_batches (
          org_id, scan_type, provider, regions_json, industries_json, roles_json,
...
def _upsert_market_company(
    ...
    *,
    ...
    existing = con.execute(
        ...
        LIMIT 1
        """,
        [org_id, normalized_company_key, country, domain or None],
    ).fetchone()
    ...
    if existing:
        row = con.execute(
            ...
            SET source_provider = ?, org_id = ?, provider_org_id = ?, scan_batch_id = ?, service_key = ?, company_name = ?, website = ?, domain = ?,
            ...
            WHERE market_company_id = ? AND org_id = ?
            RETURNING market_company_id
            """,
            [
                params[0], params[1], params[2], params[3], params[4], params[5], params[7], params[8], params[9],
                params[10], params[11], params[12], params[13], params[14], params[15], params[16], params[17], params[18], params[19],
                int(existing[0]), org_id,
            ],
        ).fetchone()
    ...
    row = con.execute(
        ...
        INSERT INTO bd_market_companies (
          org_id, source_provider, provider_org_id, scan_batch_id, service_key, company_name, normalized_company_key, website, domain,
...
def _load_never_return_company_keys(con, org_id: str | None = None) -> set[str]:
    keys: set[str] = set()
    try:
        where_clause = "WHERE lower(qualification_status) = 'binned' AND company_name IS NOT NULL AND trim(company_name) <> ''"
        params: list[Any] = []
        if org_id:
            where_clause = f"WHERE org_id = ? AND lower(qualification_status) = 'binned' AND company_name IS NOT NULL AND trim(company_name) <> ''"
...
def _ensure_client_for_opportunity(con, opp: dict[str, Any], actor: str, org_id: str) -> int:
    ...
    existing = con.execute(
        "SELECT db_id FROM clients WHERE org_id = ? AND lower(trim(client_name)) = lower(trim(?)) ORDER BY db_id DESC LIMIT 1",
        [org_id, company_name],
    ).fetchone()
    if existing:
    ...
    con.execute(
        "UPDATE bd_opportunities SET client_db_id = ?, updated_at = NOW() WHERE opportunity_id = ? AND org_id = ?",
        [client_id, int(opp.get("opportunity_id")), org_id],
    )
    return client_id
...
def generate_daily_leads(body: dict = Body(default={}), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_tables(con)
            never_return_keys = _load_never_return_company_keys(con, org_id)
            ...
            if replace_existing and not preview_only:
                ...
                con.execute(
                    ...
                    WHERE org_id = ? AND bin_date = ? AND service_key IN ({placeholder})
                      AND lower(coalesce(qualification_status, 'new')) <> 'binned'
                    """,
                    [org_id, bin_day.isoformat(), *service_keys],
                )
                ...
                for item in generated:
                    ...
                    con.execute(
                        ...
                        INSERT INTO bd_ai_generated_leads (
                          org_id, bin_date, service_key, company_name, industry, country, city, website,
                        ...
                        ON CONFLICT (org_id, bin_date, service_key, company_name) DO UPDATE SET
    ...
    try:
        ...
        with get_conn() as con:
            ...
            df = con.execute(
                ...
                LIMIT ?
                """,
                [org_id, bin_day.isoformat(), limit],
            ).df()
...
def list_daily_bins(
    ...
    try:
        ...
        try:
            with get_conn() as con:
                _ensure_tables(con)
                where = ["l.org_id = ?", "l.bin_date = ?"]
                params: list[Any] = [org_id, target_day.isoformat()]
...
def qualify_generated_lead(generated_lead_id: int, body: dict = Body(default={}), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_tables(con)
            row_df = con.execute(
                "SELECT * FROM bd_ai_generated_leads WHERE generated_lead_id = ? AND org_id = ?",
                [int(generated_lead_id), org_id],
            ).df()
            ...
            if action == "funnel" and linked_lead_id is None:
                ...
                inserted = con.execute(
                    ...
                    INSERT INTO bd_leads (
                      org_id, lead_name, company_name, contact_name, email, phone, country, industry, source,
...
def convert_lead_to_opportunity(lead_id: int, body: dict = Body(default={}), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_tables(con)
            lead_df = con.execute("SELECT * FROM bd_leads WHERE lead_id = ? AND org_id = ?", [int(lead_id), org_id]).df()
            ...
            row = con.execute(
                ...
                INSERT INTO bd_opportunities (
                  org_id, lead_id, opportunity_name, company_name, contact_name, email, phone, country, industry,
            ...
            opp_id = int(row[0])
            con.execute(
                "UPDATE bd_leads SET status='converted', converted_at=NOW(), converted_to_opportunity_id=?, updated_at=NOW() WHERE lead_id=? AND org_id = ?",
                [opp_id, int(lead_id), org_id],
            )
...
def update_opportunity(opportunity_id: int, body: dict = Body(...), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_tables(con)
            exists = con.execute("SELECT 1 FROM bd_opportunities WHERE opportunity_id = ? AND org_id = ?", [int(opportunity_id), org_id]).fetchone()
            if not exists:
            ...
            con.execute(f"UPDATE bd_opportunities SET {', '.join(updates)} WHERE opportunity_id = ? AND org_id = ?", [*params, org_id])
            return {"ok": True}
    except HTTPException:
        raise
    except Exception as e:
...
def convert_opportunity_to_client(opportunity_id: int, _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            ...
            client_id = _ensure_client_for_opportunity(con, opp, actor, org_id)
    ...
    try:
        ...
        with get_conn() as con:
            ...
            row = con.execute(
                """
                INSERT INTO quotes (
                  org_id, client_db_id, quote_date, valid_to, salesperson, currency_code, description, notes, status,
            ...
            con.execute(
                ...
                INSERT INTO quote_lines (
                  org_id, quote_id, line_type, sort_order, description, qty, unit_price_ex_vat, amount_ex_vat, vat_rate_pct, is_selected
                )
            ...
            con.execute(
                "UPDATE bd_opportunities SET quote_id = ?, client_db_id = ?, updated_at = NOW() WHERE opportunity_id = ? AND org_id = ?",
                [int(quote_id), int(client_id), int(opportunity_id), org_id],
            )
    ...
    try:
        ...
        with get_conn() as con:
            ...
            row = con.execute(
                """
                INSERT INTO jobs (
                  org_id, client_db_id, job_type_id, job_type, job_number, title, reporting_year,
            ...
            con.execute("UPDATE jobs SET job_number = ? WHERE job_id = ? AND org_id = ?", [job_number, int(job_id), org_id])
            con.execute(
                """
                ...
                """,
                [int(job_id), int(client_id), int(opportunity_id), org_id],
            )
        ...
        with get_conn() as con:
            ...
            try:
                agg_df = con.execute(
                    ...
                    WHERE org_id = ? AND lower(coalesce(status, 'open')) = 'open'
                    GROUP BY stage_id
                    """
                    ,
                    [org_id],
            ...
            try:
                lead_row = con.execute(
                    "SELECT COUNT(*) FROM bd_leads WHERE org_id = ? AND lower(coalesce(status, 'new')) IN ('new','qualified','contacted')",
                    [org_id],
                ).fetchone()
            except Exception:
                lead_row = None
