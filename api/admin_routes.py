...
MISSING_DATA_FIELDS: dict[str, dict[str, dict[str, object]]] = {
    "client": {
        ...
        "benchmark_year": {"label": "Baseline Year", "column": "benchmark_year", "type": "integer"},
        "benchmark_period_start": {"label": "Baseline Period Start", "column": "benchmark_period_start", "type": "date"},
        "benchmark_period_end": {"label": "Baseline Period End", "column": "benchmark_period_end", "type": "date"},
...
def _ensure_portfolios_lookup_table(con, org_id: str | None) -> None:
    """Ensure portfolios lookup exists with global NZI/NZN defaults (org_id=NULL)."""
    try:
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS portfolios_lookup (
              ...
              portfolio_owner_client_db_id INTEGER,
              is_active BOOLEAN NOT NULL DEFAULT TRUE
            )
            """
        )
        con.execute("ALTER TABLE portfolios_lookup ADD COLUMN IF NOT EXISTS org_id TEXT")
        con.execute("ALTER TABLE portfolios_lookup ADD COLUMN IF NOT EXISTS portfolio_owner_client_db_id INTEGER")
        # Seed NZI and NZN as global defaults (org_id=NULL) — checked by name only
        for _pname in ("NZI", "NZN"):
            con.execute(
                """
...
def _ensure_job_file_types_lookup_table(con) -> None:
    """Ensure job file types lookup exists with the default upload categories."""
    try:
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS job_file_types_lookup (
              file_type_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
              file_type_key VARCHAR NOT NULL UNIQUE,
              display_name VARCHAR NOT NULL UNIQUE,
              storage_folder_key VARCHAR NOT NULL DEFAULT 'client-provided',
              sort_order INTEGER NOT NULL DEFAULT 0,
              is_active BOOLEAN NOT NULL DEFAULT TRUE
            )
            """
        )
        con.execute("ALTER TABLE job_file_types_lookup ADD COLUMN IF NOT EXISTS file_type_key VARCHAR")
        con.execute("ALTER TABLE job_file_types_lookup ADD COLUMN IF NOT EXISTS display_name VARCHAR")
        con.execute("ALTER TABLE job_file_types_lookup ADD COLUMN IF NOT EXISTS storage_folder_key VARCHAR DEFAULT 'client-provided'")
        con.execute("ALTER TABLE job_file_types_lookup ADD COLUMN IF NOT EXISTS sort_order INTEGER DEFAULT 0")
        con.execute("ALTER TABLE job_file_types_lookup ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE")

        default_file_types = [
            ("client_provided", "Client Provided (Evidence)", "client-provided", 10, True),
            ("generated_report", "Generated Report", "generated-reports", 20, True),
        ]
        for file_type_key, display_name, storage_folder_key, sort_order, is_active in default_file_types:
            con.execute(
                """
                INSERT INTO job_file_types_lookup (file_type_key, display_name, storage_folder_key, sort_order, is_active)
                SELECT %s, %s, %s, %s, %s
                WHERE NOT EXISTS (
                  SELECT 1 FROM job_file_types_lookup WHERE lower(file_type_key) = lower(%s)
                )
                """,
                [file_type_key, display_name, storage_folder_key, sort_order, is_active, file_type_key],
            )
    except Exception:
        pass

def _ensure_governance_subjects_lookup_table(con) -> None:
    """Ensure governance subjects lookup exists with standard defaults."""
    _GOVERNANCE_SUBJECTS_DEFAULTS = ["Policies", "Responsibilities"]
    try:
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS governance_subjects_lookup (
              governance_subject_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
              name VARCHAR NOT NULL UNIQUE,
              is_active BOOLEAN NOT NULL DEFAULT TRUE
            )
            """
        )
        for subject in _GOVERNANCE_SUBJECTS_DEFAULTS:
            con.execute(
                """
                INSERT INTO governance_subjects_lookup (name)
                SELECT %s WHERE NOT EXISTS (
                  SELECT 1 FROM governance_subjects_lookup WHERE lower(name) = lower(%s)
                )
                """,
                [subject, subject],
            )
...
def _ensure_referrals_lookup_table(con) -> None:
    """Ensure the client-referral-source lookup exists with standard defaults."""
    _REFERRALS_DEFAULTS = ["Net Zero Nation", "Direct"]
    try:
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS referrals_lookup (
        ...
        for name in _REFERRALS_DEFAULTS:
            con.execute(
                """
                INSERT INTO referrals_lookup (name)
                SELECT %s WHERE NOT EXISTS (
                  SELECT 1 FROM referrals_lookup WHERE lower(name) = lower(%s)
                )
                """,
                [name, name],
            )
...
def _ensure_client_teams_lookup_table(con) -> None:
    """Ensure the SRS/action-owner team lookup exists with initial teams."""
    default_teams = ["HR", "Finance", "Sales and Marketing", "Operations", "Board"]
    try:
        con.execute(
            """
...
def _ensure_action_categories_lookup_table(con) -> None:
    """Ensure action categories lookup exists with standard defaults."""
    from services.report_actions import DEFAULT_ACTION_CATEGORIES
    try:
        con.execute(
            """
...
def _ensure_lookup_table(con, table_name: str, org_id: str | None = None) -> None:
    ...
    elif table_name == "action_categories_lookup":
        _ensure_action_categories_lookup_table(con)
    elif table_name == "governance_subjects_lookup":
        _ensure_governance_subjects_lookup_table(con)
    elif table_name == "referrals_lookup":
        _ensure_referrals_lookup_table(con)
    elif table_name == "client_teams_lookup":
        _ensure_client_teams_lookup_table(con)
