...
def _ensure_time_tracking_schema(con) -> None:
    ...
    try:
        ...
        if default_org_id is not None:
            con.execute(
                "UPDATE time_subjects SET org_id = ? WHERE org_id IS NULL",
                [default_org_id],
            )
            con.execute(
                "UPDATE time_logs SET org_id = ? WHERE org_id IS NULL",
                [default_org_id],
            )
    except Exception:
        pass
...
def create_time_log(
    ...
    try:
        ...
        with get_conn() as con:
            ...
            result = con.execute(
                """
                INSERT INTO time_logs (job_id, user_id, subject, work_date, minutes, notes, org_id)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                RETURNING time_id
                """,
                [int(job_id), user_id, subject, work_date, int(minutes), notes, org_id]
            ).fetchone()
...
def update_time_subject(
    ...
    try:
        ...
        with get_conn() as con:
            _ensure_time_tracking_schema(con)
            existing = con.execute(
                "SELECT subject_id FROM time_subjects WHERE subject_id = ? AND org_id = ?",
                [subject_id, org_id]
            ).fetchone()
            if not existing:
            ...
            con.execute(
                ...
                """,
                [float(budget_hours), subject_id, org_id]
            )
