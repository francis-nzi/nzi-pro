...
def _ensure_quote_tables(con) -> None:
    ...
    try:
        ...
        if default_org_id:
            for table_name in ("quotes", "quote_lines", "quote_email_log", "invoices", "invoice_email_log", "invoice_lines", "job_other_costs"):
                con.execute(
...
def _quote_client_row(con, quote_id: int):
    return con.execute(
        """
        SELECT q.quote_id, q.client_db_id, c.org_id
        FROM quotes q
        JOIN clients c ON c.db_id = q.client_db_id
        WHERE q.quote_id = %s
        """,
        [int(quote_id)],
    ).fetchone()
...
def _invoice_client_row(con, invoice_id: int):
    return con.execute(
        """
        SELECT i.invoice_id, i.client_db_id, c.org_id
        FROM invoices i
        JOIN clients c ON c.db_id = i.client_db_id
...
def _job_client_row(con, job_id: int):
    return con.execute(
        """
        SELECT j.job_id, j.client_db_id, c.org_id
        FROM jobs j
...
def _require_client_org_row(con, client_id: int, org_id: str):
    return con.execute(
        "SELECT db_id, client_name FROM clients WHERE db_id = %s AND org_id = %s",
        [int(client_id), org_id],
    ).fetchone()


def _require_quote_org_row(con, quote_id: int, org_id: str):
    return con.execute(
        """
        SELECT q.quote_id, q.client_db_id
        FROM quotes q
        JOIN clients c ON c.db_id = q.client_db_id
        WHERE q.quote_id = %s AND c.org_id = %s
        """,
        [int(quote_id), org_id],
    ).fetchone()


def _require_invoice_org_row(con, invoice_id: int, org_id: str):
    return con.execute(
        """
        SELECT i.invoice_id, i.client_db_id, i.quote_id, i.job_id
        FROM invoices i
        JOIN clients c ON c.db_id = i.client_db_id
        WHERE i.invoice_id = %s AND c.org_id = %s
        """,
        [int(invoice_id), org_id],
    ).fetchone()


def _require_job_org_row(con, job_id: int, org_id: str):
    return con.execute(
        """
        SELECT j.job_id, j.client_db_id
        FROM jobs j
        JOIN clients c ON c.db_id = j.client_db_id
...
def _invoice_lines_for(con, invoice_id: int, org_id: str | None = None) -> list[dict[str, Any]]:
    df = con.execute(
        ...
        """ + (" AND il.org_id = %s" if org_id is not None else "") + """
        ORDER BY COALESCE(il.sort_order, 0), il.invoice_line_id
        """,
        [int(invoice_id), *( [org_id] if org_id is not None else [] )],
    ).df()
...
def _write_invoice_lines(con, invoice_id: int, lines: list[dict[str, Any]], org_id: str | None = None) -> None:
    con.execute(
        "DELETE FROM invoice_lines WHERE invoice_id = %s" + (" AND org_id = %s" if org_id is not None else ""),
        [int(invoice_id), *( [org_id] if org_id is not None else [] )],
    )
    for idx, line in enumerate(lines):
        ...
        con.execute(
            ...
            INSERT INTO invoice_lines (
              ...
              qty, unit_price_ex_vat, amount_ex_vat, vat_rate_id, vat_rate_pct, notes, org_id
            )
...
def _serialize_invoice(con, invoice_id: int, org_id: str | None = None) -> dict[str, Any]:
    row = con.execute(
        """
        SELECT i.invoice_id, i.client_db_id, i.job_id, i.quote_id, i.invoice_number, i.invoice_date, i.due_date,
        ...
        JOIN clients c ON c.db_id = i.client_db_id
        WHERE i.invoice_id = %s
        """ + (" AND c.org_id = %s" if org_id is not None else ""),
        [int(invoice_id), *( [org_id] if org_id is not None else [] )],
    ).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Invoice not found")
    lines = _invoice_lines_for(con, int(invoice_id), org_id)
    if lines:
    ...
    if quote_id is not None:
        q_row = con.execute(
            "SELECT attention, bill_to, job_number FROM quotes q JOIN clients c ON c.db_id = q.client_db_id WHERE q.quote_id = %s" + (" AND c.org_id = %s" if org_id is not None else ""),
            [int(quote_id), *( [org_id] if org_id is not None else [] )],
        ).fetchone()
        if q_row:
...
def _serialize_quote(con, quote_id: int, org_id: str | None = None) -> dict[str, Any]:
    q = con.execute(
        """
        SELECT q.quote_id, q.client_db_id, q.contact_id, q.quote_number, q.quote_date, q.valid_to, q.salesperson,
               q.payment_term_id, pt.name AS payment_term_name, q.currency_code, q.description, q.notes, q.status, q.revision_of_quote_id,
               q.job_number, q.attention, q.bill_to, q.created_at, q.updated_at
        FROM quotes q
        JOIN clients c ON c.db_id = q.client_db_id
        LEFT JOIN payment_terms_lookup pt ON pt.term_id = q.payment_term_id
        WHERE q.quote_id = %s
        """ + (" AND c.org_id = %s" if org_id is not None else ""),
        [int(quote_id), *( [org_id] if org_id is not None else [] )],
    ).fetchone()
    ...
    df = con.execute(
        ...
        WHERE ql.quote_id = %s
        """ + (" AND ql.org_id = %s" if org_id is not None else "") + """
...
def get_quote(quote_id: int, _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_quote_tables(con)
            return _serialize_quote(con, int(quote_id), org_id)
    except HTTPException:
        raise
    except Exception as e:
...
def _write_lines(con, quote_id: int, lines: list[dict[str, Any]], org_id: str | None = None) -> None:
    con.execute(
        "DELETE FROM quote_lines WHERE quote_id = %s" + (" AND org_id = %s" if org_id is not None else ""),
        [int(quote_id), *( [org_id] if org_id is not None else [] )],
    )
    for idx, line in enumerate(lines):
        qty = _safe_float(line.get("qty"), 0.0)
        ...
        con.execute(
            ...
            INSERT INTO quote_lines (
              ...
              qty, unit_price_ex_vat, amount_ex_vat, vat_rate_id, vat_rate_pct, is_selected, notes, org_id
            )
...
def create_quote(client_id: int, body: dict = Body(...), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            ...
            row = con.execute(
                "SELECT MAX(quote_id) FROM quotes WHERE client_db_id = %s AND org_id = %s",
                [int(client_id), org_id],
            ).fetchone()
            ...
            _write_lines(con, quote_id, lines, org_id)

            return _serialize_quote(con, quote_id, org_id)
    except HTTPException:
        raise
    except Exception as e:
        ...
        with get_conn() as con:
            ...
            if updates:
                ...
                con.execute(
                    ...
                    SET {', '.join(updates)}
                    WHERE quote_id = %s
                      AND client_db_id IN (SELECT db_id FROM clients WHERE org_id = %s)
                    """,
                    params + [org_id],
                )
            ...
            if "lines" in body:
                ...
                if not isinstance(lines, list):
                    raise HTTPException(status_code=400, detail="lines must be an array")
                _write_lines(con, int(quote_id), lines, org_id)
    ...
    try:
        ...
        with get_conn() as con:
            ...
            con.execute(
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'Draft', %s, %s, %s, %s, NOW(), NOW(), %s)
                """,
                [
                    int(q[1]),
                    _safe_int(q[2], None),
            ...
            _write_lines(con, new_quote_id, lines, org_id)
            return _serialize_quote(con, new_quote_id, org_id)
    except HTTPException:
        raise
    except Exception as e:
...
def email_quote(quote_id: int, body: dict = Body(...), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            ...
            con.execute(
                """
                INSERT INTO quote_email_log (quote_id, sent_to, subject, body, sent_by, sent_at, org_id)
                VALUES (%s, %s, %s, %s, %s, NOW(), %s)
                """,
                [int(quote_id), sent_to, subject, message, sender, org_id],
            )
            con.execute(
                """
                UPDATE quotes
...
def get_quote_pdf(quote_id: int, _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_quote_tables(con)
            quote = _serialize_quote(con, int(quote_id), org_id)
    ...
    try:
        ...
        with get_conn() as con:
            ...
            con.execute(
                ...
                VALUES (%s, %s, %s, %s, %s, NOW(), %s)
                """,
                [int(quote_id), to_email, rendered["subject"], rendered["body_html"], sender, org_id],
            )
...
def list_job_invoices(job_id: int, _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_quote_tables(con)
            job_exists = _require_job_org_row(con, int(job_id), org_id)
            if not job_exists:
...
def create_invoice(client_id: int, body: dict = Body(...), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            ...
            invoice_id = int(row[0])
            if lines:
                _write_invoice_lines(con, invoice_id, lines, org_id)
                con.execute(
                    """
                    UPDATE invoices
                    SET subtotal = %s, vat = %s, total = %s, updated_at = NOW()
                    WHERE invoice_id = %s AND org_id = %s
                    """,
                    [computed_totals["subtotal"], computed_totals["vat"], computed_totals["total"], int(invoice_id), org_id],
                )
            return _serialize_invoice(con, invoice_id, org_id)
    except HTTPException:
        raise
    except Exception as e:
        ...
        with get_conn() as con:
            ...
            con.execute(
                ...
                INSERT INTO job_other_costs (
                  ...
                  is_vatable, vat_rate_pct, vat_amount, total_inc_vat, supplier, incurred_date, notes, org_id
                )
            ...
            created_df = con.execute(
                ...
                SELECT
                  ...
                  supplier, incurred_date, notes, created_at, updated_at
                FROM job_other_costs
                WHERE other_cost_id = %s AND org_id = %s
                """,
...
def update_invoice(invoice_id: int, body: dict = Body(...), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            ...
            exists = con.execute(
                """
                SELECT i.invoice_id, i.xero_invoice_id
                FROM invoices i
                JOIN clients c ON c.db_id = i.client_db_id
                WHERE i.invoice_id = %s AND c.org_id = %s
                """,
            ...
            if "lines" in body:
                ...
                if not isinstance(lines, list):
                    raise HTTPException(status_code=400, detail="lines must be an array")
                _write_invoice_lines(con, int(invoice_id), lines, org_id)
            ...
            return _serialize_invoice(con, int(invoice_id), org_id)
    except HTTPException:
        raise
    except Exception as e:
...
def delete_invoice(invoice_id: int, _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            ...
            if not exists:
                raise HTTPException(status_code=404, detail="Invoice not found")
            con.execute("DELETE FROM invoice_lines WHERE invoice_id = %s AND org_id = %s", [int(invoice_id), org_id])
    ...
    try:
        ...
        with get_conn() as con:
            ...
            con.execute(
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NULL, 0, NOW(), NOW(), %s)
                """,
                [
...
def get_invoice_pdf(invoice_id: int, _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            _ensure_quote_tables(con)
            invoice = _serialize_invoice(con, int(invoice_id), org_id)
...
def email_invoice_pdf(invoice_id: int, body: dict = Body(...), _user: dict = Depends(_current_user)):
    try:
        ...
        with get_conn() as con:
            ...
            con.execute(
                """
                INSERT INTO invoice_email_log (invoice_id, sent_to, subject, body, sent_by, sent_at, org_id)
                VALUES (%s, %s, %s, %s, %s, NOW(), %s)
                """,
                [int(invoice_id), to_email, rendered["subject"], rendered["body_html"], sender, org_id],
            )
    ...
    try:
        ...
        with get_conn() as con:
            ...
            time_df = con.execute(
                ...
                WHERE j.client_db_id = %s AND c.org_id = %s AND tl.org_id = %s
                """,
            ...
            other_df = con.execute(
                ...
                JOIN clients c ON c.db_id = j.client_db_id
                WHERE j.client_db_id = %s AND c.org_id = %s AND oc.org_id = %s
                """,
                [int(client_id), org_id, org_id],
            ).df()
            other_subtotal = 0.0
        ...
        with get_conn() as con:
            ...
            if client_id is not None and job_number:
                quote_ids_df = con.execute(
                    ...
                    """,
                    [int(client_id), org_id, job_number],
                ).df()
                if quote_ids_df is not None and not quote_ids_df.empty:
                    for _, r in quote_ids_df.iterrows():
                        qid = _safe_int(r.get("quote_id"), None)
            ...
            time_df = con.execute(
                ...
                JOIN clients c ON c.db_id = j.client_db_id
                LEFT JOIN job_types jt ON jt.job_type_id = j.job_type_id
                WHERE tl.job_id = %s AND c.org_id = %s AND tl.org_id = %s
                """,
