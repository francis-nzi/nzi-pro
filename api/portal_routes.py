...
def portal_live_report_data(job_id: int, current_user: dict = Depends(portal_user_dep)):
    ...
    with org_context(_org_id):
        try:
            ...
            if not job_data:
                raise HTTPException(status_code=404, detail="Job not found")
            job_family = str(job_data.get("job_family") or "crp").strip().lower()
            if job_family and job_family != "crp":
                raise HTTPException(
                    status_code=404,
                    detail="Report view is only available for Carbon Reduction Plan jobs.",
                )
