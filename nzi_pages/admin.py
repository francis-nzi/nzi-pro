...
def render():
    ...
    with t2:
        ...
        _ensure_job_status_lookup_schema()
        l1, l2, l3, l4, l5, l6, l7 = st.tabs([
            "Job Types",
            "Job Statuses",
            "VAT Rates",
            "Payment Terms",
        ...
        with l2:
            _lookup_editor(
                "job_statuses_lookup",
                ["status_id", "name", "sort_order", "is_active"],
                id_col="status_id",
                name_col="name",
                title="Job Statuses",
            )
...
def _ensure_job_status_lookup_schema():
    try:
        with get_conn() as con:
            if db_backend() == "postgres":
                con.execute(
                    """
                    CREATE TABLE IF NOT EXISTS public.job_statuses_lookup (
