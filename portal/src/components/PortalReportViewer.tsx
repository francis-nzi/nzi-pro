    ...
    <div className="h-[320px]">
      <ResponsiveContainer width="100%" height={320}>
        <LineChart data={chartData} margin={{ top: 5, right: 24, left: 8, bottom: 5 }}>
          ...
          <Line type="monotone" dataKey="actual_total" name="Total (actual)" stroke="#0f766e" strokeWidth={3} dot={{ r: 5 }} activeDot={{ r: 6 }} connectNulls={false} />
          <Line type="monotone" dataKey="actual_s1" name="Scope 1 (actual)" stroke={SCOPE_COLORS["Scope 1"]} strokeWidth={2} dot={{ r: 3 }} connectNulls={false} />
          {hasScope2 && <Line type="monotone" dataKey="actual_s2" name="Scope 2 (actual)" stroke={SCOPE_COLORS["Scope 2"]} strokeWidth={2} dot={{ r: 3 }} connectNulls={false} />}
          <Line type="monotone" dataKey="actual_s3" name="Scope 3 (actual)" stroke={SCOPE_COLORS["Scope 3"]} strokeWidth={2} dot={{ r: 3 }} connectNulls={false} />
          <Line type="monotone" dataKey="target_total" name="Total (target)" stroke="#0f766e" strokeWidth={2} strokeDasharray="5 4" dot={false} />
          <Line type="monotone" dataKey="target_s1" name="Scope 1 (target)" stroke={SCOPE_COLORS["Scope 1"]} strokeWidth={1.5} strokeDasharray="5 4" dot={false} />
          {hasScope2 && <Line type="monotone" dataKey="target_s2" name="Scope 2 (target)" stroke={SCOPE_COLORS["Scope 2"]} strokeWidth={1.5} strokeDasharray="5 4" dot={false} />}
          <Line type="monotone" dataKey="target_s3" name="Scope 3 (target)" stroke={SCOPE_COLORS["Scope 3"]} strokeWidth={1.5} strokeDasharray="5 4" dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
  ...
  const chartData = useMemo(() => {
    ...
    return years.map(year => {
      ...
      metricEntries.forEach(entry => {
        row[`${entry.label}_actual`] = actual ? parseFloat(((actual.total * entry.divider) / entry.value).toFixed(3)) : null;
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="mt-6">
                  ...
                  <div className="overflow-x-auto">
                    <div className="overflow-hidden rounded-lg border border-gray-200 min-w-[640px]">
                      ...
                      {rows.map((row, i) => (
                        <div key={i} className={`grid grid-cols-[1fr_80px_110px_140px_60px_60px_80px] border-b border-gray-100 last:border-0 px-3 py-2 ${i % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                          <span className="text-xs text-gray-700">{row.site_name ?? "Unassigned"}</span>
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          <p className="text-sm text-gray-700 leading-relaxed">
            To achieve our net zero commitment, {data.job_data.client_name ?? "the organisation"} has identified the following key areas for emissions reduction. These actions will be implemented in phases over the coming years.
          </p>
          <div>
            <p className="text-sm font-bold text-gray-800 mb-3">Planned Initiatives</p>
            <div className="overflow-hidden rounded-lg border border-gray-200">
              <div className="grid items-center px-3 py-2.5" style={{ gridTemplateColumns: "140px 1fr 140px 2fr", backgroundColor: "#8abb8a" }}>
              ...
              {(job_actions?.total_actions ?? 0) > 0
                ? (job_actions?.grouped ?? []).flatMap((group) =>
                    (group.items ?? []).map((item, ii) => (
                      <div key={`${group.term}-${ii}`} className="grid items-start border-t border-gray-100 px-3 py-3" style={{ gridTemplateColumns: "140px 1fr 140px 2fr" }}>
                        <div className="pt-0.5">
                          <span className="inline-flex items-center rounded-full border border-green-400 bg-green-50 px-2.5 py-0.5 text-xs font-medium text-green-700">{group.label ?? group.term}</span>
                        </div>
                        <span className="pr-4 text-sm font-semibold text-gray-800">{String(item.action_name ?? "")}</span>
                        <span className="pr-4 text-sm text-gray-600">{String(item.action_category ?? "")}</span>
                        <span className="text-sm text-gray-600">{String(item.description ?? "")}</span>
                      </div>
                    ))
                  )
                : (
                  <div className="grid items-start border-t border-gray-100 px-3 py-3" style={{ gridTemplateColumns: "140px 1fr 140px 2fr" }}>
                    <div className="pt-0.5">
                      <span className="inline-flex items-center rounded-full border border-green-400 bg-green-50 px-2.5 py-0.5 text-xs font-medium text-green-700">Short term</span>
                    </div>
                    <span className="pr-4 text-sm font-semibold text-gray-800">Action plan in development</span>
                    <span className="pr-4 text-sm text-gray-600">General</span>
                    <span className="text-sm text-gray-600">Actions will be added by the NZI team prior to final issue.</span>
                  </div>
                )}
            </div>
          </div>
