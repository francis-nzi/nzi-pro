...
export default function ClientInvoicesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError("");
      try {
        const res = await fetch(`${baseUrl}/clients/${clientId}/invoices`, { credentials: "include" });
        if (!res.ok) {
      ...
      <div className="mx-auto w-full max-w-7xl px-6 py-10">
        <PageHeader
          ...
          actions={
            <>
              <Button asChild>
                <Link href={`/clients/${clientId}/invoices/new`}>+ Add Invoice</Link>
              </Button>
                ...
                <div className="divide-y">
                  {items.map((row) => (
                    <div key={row.invoice_id} className="grid grid-cols-12 gap-2 px-3 py-2 text-sm">
                      ...
                      <div className="col-span-2">{row.invoice_date ? new Date(row.invoice_date).toLocaleDateString("en-GB") : "-"}</div>
                      ...
                      <div className="col-span-2 text-right">
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/clients/${clientId}/invoices/${row.invoice_id}`}>Open</Link>
                        </Button>
                      </div>
                    </div>
                  ))}
